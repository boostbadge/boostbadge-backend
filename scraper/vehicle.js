...
async function createVehicle(wpVehicle) {
  const user = await models.User.findOne({ where: { wpId: wpVehicle.ownerId } });
  return models.Vehicle.create({
    ...
    color: wpVehicle.color,
    trimLevel: wpVehicle.trimLevel,
    engine: wpVehicle.engine,
    horsepower: wpVehicle.horsepower,
    torque: wpVehicle.torque,
    fuelType: wpVehicle.fuelType,
    engineMods: wpVehicle.engineMods,
    drivetrain: wpVehicle.drivetrain,
    transmission: wpVehicle.transmission,
    interiorMods: wpVehicle.interiorMods,
    wheels: wpVehicle.wheels,
    tires: wpVehicle.tires,
    curbWeight: wpVehicle.curbWeight,
    exteriorMods: wpVehicle.exteriorMods,
    suspension: wpVehicle.suspension,
    blurb: wpVehicle.blurb,
    futurePlans: wpVehicle.futurePlans,
    wheelbase: wpVehicle.wheelbase,
    ...
    homeState: wpVehicle.homeState,
    forSale: wpVehicle.forSale,
    additionalInformation: wpVehicle.additionalInformation,
    bodyStyle: wpVehicle.bodyStyle,
  });
}
...
async function processVehicle(wpVehicle) {
  const vehicle = await models.Vehicle.findOne({ where: { wpId: wpVehicle.wpId } });
  if (vehicle) {
    const updatedVehicle = await updateVehicle(vehicle, wpVehicle);
    console.log(`Updated Vehicle: ${updatedVehicle.id} (wpId: ${updatedVehicle.wpId})`);
  } else {
    const createdVehicle = await createVehicle(wpVehicle);
    console.log(`Created Vehicle: ${createdVehicle.id} (wpId: ${createdVehicle.wpId})`);
  }
}
...
export async function scrapeVehicles() {
  ...
  for (const wpVehicle of wpVehicles) {
    await processVehicle(wpVehicle);
  }
